...
public class FluidBaseCableBlock extends Block {
    ...
    public FluidBaseCableBlock(Material materialIn) {
    ...
    public void onBlockHarvested(World world, BlockPos pos, IBlockState state, EntityPlayer player) {
        FluidCableTileEntiity tileEntiity = (FluidCableTileEntiity) world.getTileEntity(pos);
        if (tileEntiity != null && tileEntiity.getFluidAmount(null) > 0) {
            //
        }
    }
    @Override
    ...
    public RayTraceResult collisionRayTrace(IBlockState state, World world, BlockPos pos, Vec3d start, Vec3d end) {
        FluidCableTileEntiity tileEntiity = (FluidCableTileEntiity) world.getTileEntity(pos);
        List<AxisAlignedBB> boxes = tileEntiity.boxes;
        ...
        this.lookingSide = -1;
        for (int i = 0; i < 6; i++) {
            if (this.boundbox == tileEntiity.uncoveredBoxes[i]) {
                this.lookingSide = i;
            }
        }
    ...
    public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand, EnumFacing side, float hitX, float hitY, float hitZ) {
        if (player.inventory.getCurrentItem() != null) {
            ...
            if (item instanceof ItemShears) {
                tileEntiity.incrementSide(this.lookingSide, player, world);
